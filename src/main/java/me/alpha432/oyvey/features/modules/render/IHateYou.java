...
public class IHateYou
        extends Module {
    private static IHateYou INSTANCE = new IHateYou();
    ...
    public IHateYou() {
        super("IHateYou", "i hate you", Module.Category.RENDER, false, false, false);
        this.setInstance();
    }

    public static IHateYou getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new IHateYou();
        }
        return INSTANCE;
    }
    ...
    public void onRender3D(Render3DEvent event) {
        assert (IHateYou.mc.renderViewEntity != null);
        Vec3i playerPos = new Vec3i(IHateYou.mc.renderViewEntity.posX, IHateYou.mc.renderViewEntity.posY, IHateYou.mc.renderViewEntity.posZ);
        for (int x = playerPos.getX() - this.range.getValue(); x < playerPos.getX() + this.range.getValue(); ++x) {
            for (int z = playerPos.getZ() - this.range.getValue(); z < playerPos.getZ() + this.range.getValue(); ++z) {
                for (int y = playerPos.getY() + this.rangeY.getValue(); y > playerPos.getY() - this.rangeY.getValue(); --y) {
                    ...
                    if (!IHateYou.mc.world.getBlockState(pos).getBlock().equals(Blocks.AIR) || !IHateYou.mc.world.getBlockState(pos.add(0, 1, 0)).getBlock().equals(Blocks.AIR) || !IHateYou.mc.world.getBlockState(pos.add(0, 2, 0)).getBlock().equals(Blocks.AIR) || pos.equals(new BlockPos(IHateYou.mc.player.posX, IHateYou.mc.player.posY, IHateYou.mc.player.posZ)) && !this.renderOwn.getValue().booleanValue() || !BlockUtil.isPosInFov(pos).booleanValue() && this.fov.getValue().booleanValue())
                        continue;
                    if (IHateYou.mc.world.getBlockState(pos.north()).getBlock() == Blocks.BEDROCK && IHateYou.mc.world.getBlockState(pos.east()).getBlock() == Blocks.BEDROCK && IHateYou.mc.world.getBlockState(pos.west()).getBlock() == Blocks.BEDROCK && IHateYou.mc.world.getBlockState(pos.south()).getBlock() == Blocks.BEDROCK && IHateYou.mc.world.getBlockState(pos.down()).getBlock() == Blocks.BEDROCK) {
                    ...
                    if (!BlockUtil.isBlockUnSafe(IHateYou.mc.world.getBlockState(pos.down()).getBlock()) || !BlockUtil.isBlockUnSafe(IHateYou.mc.world.getBlockState(pos.east()).getBlock()) || !BlockUtil.isBlockUnSafe(IHateYou.mc.world.getBlockState(pos.west()).getBlock()) || !BlockUtil.isBlockUnSafe(IHateYou.mc.world.getBlockState(pos.south()).getBlock()) || !BlockUtil.isBlockUnSafe(IHateYou.mc.world.getBlockState(pos.north()).getBlock()))
                        continue;
